...
@section('content')
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="court_select" class="court_select form-control form-control-sm"
                                    ...
                                    @foreach ($district_majistrate_court as $district_majistrate_court_single)
                                    @php
                                        $selected=' ';
                                        if ($district_majistrate_court_single->id == $value['court_id']) {
                                            $selected='selected';
                                        }
                                    @endphp
                                    <option value="{{ $district_majistrate_court_single->id }}" {{ $selected }} class="dis_majis_court">
                                        {{ $district_majistrate_court_single->court_name }}</option>
                                  @endforeach

                                    @foreach ($courtlist_district as $courtlist_upazila_single)
                                        @php
                                            $selected=' ';
                                            if ($courtlist_upazila_single->id == $value['court_id']) {
                                                $selected='selected';
                                            }
                                        @endphp
                                        <option value="{{ $courtlist_upazila_single->id }}" {{ $selected }} class="dis_court">
                                            {{ $courtlist_upazila_single->court_name }}</option>
                                    @endforeach

                                    @foreach ($courtlist_upazila as $courtlist_upazila_single)
                                        @php
                                            $selected=' ';
                                            if ($courtlist_upazila_single->id == $value['court_id']) {
                                                $selected='selected';
                                        ...
                                        @endphp
                                        <option value="{{ $courtlist_upazila_single->id }}" {{ $selected }} class="upa_court">
                                            {{ $courtlist_upazila_single->court_name }}</option>
                                    @endforeach
    ...
    <script>
      ...
      $('select[name="role_select"]').on('change',function()
      {
         ...
         {
            $('.upa_court').show();
            $('.dis_majis_court').hide();
            $('.dis_court').hide();
         }
         else if(role_id == 38 || role_id == 39)
         {
            $('.dis_court').show();
            $('.dis_majis_court').hide();
            $('.upa_court').hide();
        ...
        $('.court_select').on('change', function() {
            ...
            if (role_id == 27) {
                swal.showLoading();
                $.ajax({
                    url: '{{ route('admin.doptor.management.em.dc.create') }}',
                    method: 'post',
                    data: {
                ...
                swal.showLoading();
                $.ajax({
                    url: '{{ route('admin.doptor.management.em.peshkar.dc.create') }}',
                    method: 'post',
                    data: {
                        office_name_bn: $('#' + 'office_name_bn_' + id).val(),
                        office_name_en: $('#' + 'office_name_en_' + id).val(),
            ...
            else if(role_id == 38)
            {
                swal.showLoading();
                $.ajax({
                    url: '{{ route('admin.doptor.management.adm.create') }}',
                    method: 'post',
                    data: {
                        office_name_bn: $('#' + 'office_name_bn_' + id).val(),
                        office_name_en: $('#' + 'office_name_en_' + id).val(),
            ...
            else if(role_id == 39)
            {
                swal.showLoading();
                $.ajax({
                    url: '{{ route('admin.doptor.management.adm.peshkar.dc.create') }}',
                    method: 'post',
                    data: {
                        office_name_bn: $('#' + 'office_name_bn_' + id).val(),
                        office_name_en: $('#' + 'office_name_en_' + id).val(),
