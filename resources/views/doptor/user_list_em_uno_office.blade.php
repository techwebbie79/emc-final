    ...
    <div class="card">
        <div class="card-header flex-wrap py-5">
            ...
            <div class="card-toolbar">
                <a href="{{ route('doptor.user.management.user_list.segmented.all.pick.em.uno', ['office_id' => encrypt($office_id)]) }}"
        ...
        <form class="form-inline" method="GET" action="{{ route('doptor.user.management.user_list.segmented.all.pick.em.uno.search',['office_id'=>encrypt($office_id)]) }}">
            ...
            <input type="hidden" name="" id="office_id_hidden" value="{{ $office_id }}">
              <table class="table table-striped table-hover">
               ...
               <tr>
                   ...
                   <tr>
                    <input type="hidden" name="" id="office_name_bn_{{ $increment }}" value="{{ $value['office_name_bn'] }}">
                    <input type="hidden" name="" id="office_name_en_{{ $increment }}" value="{{ $value['office_name_en'] }}">
                    <input type="hidden" name="" id="unit_name_bn_{{ $increment }}" value="{{ $value['unit_name_bn'] }}">
                    <input type="hidden" name="" id="unit_name_en_{{ $increment }}" value="{{ $value['unit_name_en'] }}">
                    <input type="hidden" name="" id="designation_bng_{{ $increment }}" value="{{ $value['designation_bng'] }}">
                    <input type="hidden" name="" id="office_id_{{ $increment }}" value="{{ $value['office_id'] }}">
                    <input type="hidden" name="" id="username_{{ $increment }}" value="{{ $value['username'] }}">
                    <input type="hidden" name="" id="employee_name_bng_{{ $increment }}" value="{{ $value['employee_name_bng'] }}">
                    
                       <td>{{ en2bn($increment)}}</td>
                      
                       <td><input type="text" class="form-control" value="{{ $value['employee_name_bng'] }}" readonly></td>
                       <td><input type="text" class="form-control" value="{{ $value['designation_bng'] }}" readonly></td>
                       <td><input type="text" class="form-control" value="{{ $value['designation_eng'] }}" readonly></td>
                       <td><select name="court_select" class="court_select form-control form-control-sm" class="form-control form-control-sm" 
                        id="{{ $increment }}">
                        ...
                        <option value="0" >কোন আদালত দেওয়া হয় নাই</option> 
                            ...
                            @endphp
                                <option value="{{ $available_court_single->id }}" {{ $selected }}>{{ $available_court_single->court_name}}</option> 
                            @endforeach
                       </select></td>
                   <td>
                   ...
                   @php  array_push($court_ids,$available_court_single->id); @endphp
                    ...
                    @endphp
                    <button class="btn-sm btn btn-primary court_name_{{ $increment }}">{{ $court_name_showing }} এনাবেল</button>
                    @else
