   ...
   public function appeal_hearing_list_old(Request $request)
    {
        ...
        $d = $request->date_start;
        $data['hearing'] = EmCauseList::orderby('id', 'DESC')
            // ->with('appeal')
   ...
   public function appeal_hearing_list(Request $request)
    {
        ...
        $d = $request->date_start;
        $hearin_date_calendar= EmAppeal::orderby('id', 'DESC')->where('next_date', '=', $d);
        ...
        {
        $hearin_date_calendar->where('court_id',globalUserInfo()->court_id)->whereIn('appeal_status', ['ON_TRIAL_DM']);
        }
        elseif(globalUserInfo()->role_id == 37)
        {

           $hearin_date_calendar->whereIn('appeal_status', ['ON_TRIAL_DM'])->where('district_id', user_district());
        }
        elseif(globalUserInfo()->role_id == 7)
        {

            $hearin_date_calendar->whereIn('appeal_status', ['ON_TRIAL_DM'])->where('assigned_adc_id',globalUserInfo()->id)->where('district_id', user_district());
        }
            

        $data['hearing']=$hearin_date_calendar->paginate(30); 
        $data['page_title'] = 'শুনানীর তারিখ: ' . en2bn(date('d-m-Y', strtotime($d))) . 'ইং';
        return view('hearing.hearing_date')->with($data);
    }
