...
class GoogleLoginController extends Controller
{
    ...
    public function nid_check_advocate_google_sso(Request $request)
    {
        //
        $data['short_news'] = News::orderby('id', 'desc')->where('news_type', 1)->where('status',1)->get();
        $data['big_news'] = News::orderby('id', 'desc')->where('news_type', 2)->where('status',1)->get();
        ...
        return view('googleSSOcitizen.nid_check_advocate')->with($data);
    }
    
    public function nid_verify_advocate_google_sso(Request $request)
    {
        $request->validate([
        ...
        if(empty($result)){
            ...
            return redirect('/advocateRegister/google/sso');
        }
    }
    public function create_advocate_google_sso(Request $request)
    {
        $SesionData = Session::get('nidData');
        $data['results'] = $SesionData;
        $cookieData = $request->cookie('Gmail_info');
        ...
        // return $data;
        return view('googleSSOcitizen.advocate_add')->with($data);
    }
