...
class DummiDataController extends Controller
{
    public function dammi_case_no()
    {
        ...
        //dd(DB::table('em_citizens')->where('id',4)->select('designation')->first()->designation);
    ...
    {
        $email_array = ['touhidulislam256@gmail.com', 'touhidul.developer.2022@gmail.com', 'touhidul.encoderit@gmail.com', 'tutul5162@gmail.com'];

        $all_citizens = DB::table('em_citizens')->select('id')->get();
        foreach ($all_citizens as $all_citizens) {
            DB::table('em_citizens')->where('id', $all_citizens->id)->update([
