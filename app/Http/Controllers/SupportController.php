...
class SupportController extends Controller
{
    ...
    public function support_form_page()
    {
        ...
        if (Auth::user()) {
            ...
            return view('support.no_auth_form');
        }

    }
    // public function support_user_create()
    // {
    //     $data['page_title'] = 'নতুন সাপোর্ট ইউজার এন্ট্রি ফরম';

    //     return view('support.add')->with($data);
    // }
    public function support_form_post(Request $request)
    {
        ...
        //$image = base64_encode(file_get_contents($request->file('file_name')));
        // dd($_FILES);
        
         // $attachments = [];
        // $attachments_wrapper_inner = [];
        // $attachments_wrapper_outer = [];

        // if (!empty($_FILES['file_name']['name'])) {
        //     foreach ($_FILES['file_name']['name'] as $key => $value) {
        //         $api_string = "data:" . $_FILES['file_name']['type'][$key] . ';base64,' . base64_encode(file_get_contents($request->file('file_name')[$key]));
        //         $attachments[$_FILES['file_name']['name'][$key]] = $api_string;

        //         array_push($attachments_wrapper_inner, $attachments);
        //     }

        //     //array_push($attachments_wrapper_outer,$attachments_wrapper_inner);
        // }

        //dd($attachments_wrapper_inner);
        
         //dd($zip_attachments_wrapper);
