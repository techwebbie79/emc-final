...
class OnlineHearingRepository
{
    public static function storeHearingKey($appealId,$shortorderid,$trialdate,$trialtime)
    {
      ...
      if($shortorderid == 10 || $shortorderid == 11 )
      {
        $case_no=DB::table('em_appeals')->select('case_no')->where('id','=',$appealId)->first();
      $short_order_name=DB::table('em_case_shortdecisions')->where('id','=',$shortorderid)->first();
