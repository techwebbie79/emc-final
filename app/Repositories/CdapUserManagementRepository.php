...
class CdapUserManagementRepository
{
    ...
    public static function create_cdap_user_with_login($data_from_cdap)
    {
        ...
        $nid_exits=DB::table('em_citizens')->where('citizen_NID',$data_from_cdap['data']['nid'])->first();
            
        
        if(!empty($nid_exits))
        {
            DB::table('em_citizens')->where('citizen_NID',$data_from_cdap['data']['nid'])->update($em_citizens_array);
            $created_citigen_ID=$nid_exits->id;
        }
        else
        {
            
            $created_citigen_ID = DB::table('em_citizens')->insertGetId($em_citizens_array);
    ...
    public static function create_cdap_user_advocate_with_login($data_from_cdap)
    {
